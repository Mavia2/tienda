...
class Formavieja extends Model
{
  ...
  public static function buscarIdFacebook($idfacebook){

    $resultadoBusqueda = DB::table('facebook')
  ...
  public static function dataface($album, $busca){
    ...
    $apialbum="https://graph.facebook.com/v2.11/$album?access_token=EAAUJuPkcQtkBAAaM9R5q2YZAmSFXeJDh5NMZBskyZBdXiahvOmj54j6ryDWybVJlBUb75avyM4aJ4x2vUiCtZAF4vAOV9OclIK6ZACPOKTbcUMNMdkGrc7s8av8g6VoHCebU3ArjUlYtyfZCZBEm3eRjZBDcUpKvfd86qh7nj0N3UgZDZD&fields=photos{name,picture,link,images}";
    $tools = json_decode(file_get_contents($apialbum));    
    $data = $tools->photos->data;     
    $next = $tools->photos->paging->next;
    $tools1 = json_decode(file_get_contents(str_replace ("limit=25","limit=100",$next))); 
    ...
    foreach ($data as $key => $value) {        
       $face[]=['pic'=>$value->picture, 'link'=>$value->link, 'code'=>substr($value->name,0,5), 'name'=>$value->name, 'idfacebook'=>$value->id, 'img'=>$value->images[1]->source];
      }
